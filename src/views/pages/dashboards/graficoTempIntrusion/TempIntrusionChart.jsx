...
const TempIntrusionChart = ({ shipment_id }) => {
  ...
  const [getShipment, { error: contentError, data: contentData }] =
    useLazyQuery(GET_SHIPMENT_DETAIL);
  let branch_departure = company_detail?.company?.branches?.map(
    (b) => b.branch_id === contentData?.shipment?.origin_id && b.name
  );
  let branch_arrival = company_detail?.company?.branches?.map(
    (b) => b.branch_id === contentData?.shipment?.destination_id && b.name
  );
  ...
  let data = [
    {
      value: contentData?.shipment?.temperature_readings?.[0]?.cv
        ? contentData?.shipment?.temperature_readings?.[0]?.cv
        : contentData?.shipment?.temperature_readings?.[0]?.value,
      counter: 0,
      formattedCounter: "0d 0h 0m 0s",
      intrusion: 0,
      timestamp: contentData?.shipment?.temperature_readings?.[0]?.timestamp,
      millisec: Date.parse(
        contentData?.shipment?.temperature_readings?.[0]?.timestamp
      ),
  ...
  useEffect(() => {
    ...
    for (
      let i = 0;
      i < contentData?.shipment?.temperature_readings?.length;
      i++
    ) {
      ...
      for (let j = 0; j < contentData?.shipment?.intrusions?.length; j++) {
        if (
          contentData?.shipment?.temperature_readings?.[i]?.counter ===
          contentData?.shipment?.intrusions[j].init_counter
        ) {
          hasIntrusion = true;
          break;
        } else if (
          contentData?.shipment?.temperature_readings?.[i]?.counter ===
          contentData?.shipment?.intrusions[j].final_counter
        ) {
          hasIntrusion = false;
        }
      }
      data.push({
        value: contentData?.shipment?.temperature_readings?.[i]?.cv
          ? contentData?.shipment?.temperature_readings?.[i]?.cv
          : contentData?.shipment?.temperature_readings?.[i]?.value,
        ...
        millisec: Date.parse(
          contentData?.shipment?.temperature_readings[i]?.timestamp
        ),
      });
    }
    ...
    <div className={styles.allContainer}>
      <div className={styles.firstLine}>
        <div className={styles.imageAndTitle}>
          ...
          <p className={styles.title}> Temperature/intrusion register</p>
        </div>
        <div className={styles.infoBox}>
          ...
          <div className={styles.eachBox}>
            <div className={styles.infoIDTitle}>DEPARTURE</div>
            <div className={styles.info}>{departure}</div>
          </div>
          <div className={styles.eachBox}>
            <div className={styles.infoTitle}>STATUS</div>
            {contentData?.shipment?.status === "SUCCESSFUL" && (
              <div className={styles.info}>Successful</div>
            )}
            {contentData?.shipment?.status === "UNCERTAIN" && (
              <div className={styles.info}>Uncertain</div>
            )}
            {contentData?.shipment?.status === "FAILED" && (
              <div className={styles.info}>Failed</div>
            )}
          </div>
          <div className={styles.eachBox}>
            <div className={styles.infoTitle}>DESTINATION</div>
